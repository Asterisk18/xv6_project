...
int nextpid = 1;
// pid_lock is used during process allocation, 
// it ensures that no new pid is allocated until 
// the allocation of current pid is completed
struct spinlock pid_lock;

// a
struct spinlock queue_lock;
// prior declarations for queue functions
...
// wait_lock preserves relationships
// always used before performing any
// operation related to parent-child
// perform any other lock after acquiring this lock
    ...
    for(int level = 0; level < NLEVEL; level++){
      while(queues[level].count){ // while the current level is not empty
        ...
        if(p->state == RUNNABLE) {
          ...
          // setting values to zero before every scheduling
          p->delta_s = 0;
          p->curr_ticks = 0;

          // using saved value for process level, because using p->level 
          // might give the value after demotion of level, 
          // thus resulting in wrong accounting of ticks
          ...
          // if the poped process is not runnable then
          // simply drop it, it will be added again to 
          // the queue when wakeup() is called upon it
        }
        release(&p->lock);
        if(found) break;
      }
