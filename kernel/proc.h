...
// a
#define NLEVEL 4
enum levels {LEVEL0=0, LEVEL1, LEVEL2, LEVEL3};
...
struct proc {
  ...
  int syscount;
  enum levels level;
  int delta_s; // number of syscalls made in current scheduling
  int curr_ticks; // ticks consumed in current scheduling
...
// a
struct mlfqinfo {
  enum levels level; // current queue level
  int ticks[NLEVEL]; // total ticks consumed at each level
  int times_scheduled; // number of times the process has been scheduled
